...
namespace HighlightPlus {
    ...
    public partial class HighlightEffect : MonoBehaviour {
                    ...
                    if (useSmoothBlend && independentFullScreenNotExecuted) {
                        ...
                        for (int l = 0; l < mesh.subMeshCount; l++) {
                            ...
                            if (outlineQuality.UsesMultipleOffsets()) {
                                ...
                                fxMatOutline[0].SetVector("_OutlineDirection", Vector4.zero);
                ...
                if (fading == FadingState.FadingIn) {
                    ...
                    if (fade < 0f) {
                        ...
                        if (OnObjectHighlightEnd != null) {
                            OnObjectHighlightEnd(target.gameObject);
                        }
                    ...
                    if (overlay > 0) {
                        for (int l = 0; l < mesh.subMeshCount; l++) {
                            ...
                            if (l < rms[k].fxMatOverlay.Length && rms[k].fxMatOverlay[l] != null) {
                                ...
                                rms[k].fxMatOverlay[l].SetVector("_OverlayData", new Vector4(overlayAnimationSpeed, overlayMinIntensity, overlayBlending));
                        ...
                        for (int glowPass = 0; glowPass < glowPasses.Length; glowPass++) {
                            if (glowQuality.UsesMultipleOffsets()) {
                                ...
                                if (allowGPUInstancing) {
                                    ...
                                    fxMatGlow[matIndex].SetColor("_GlowColor", dataColor);
                                    fxMatGlow[matIndex].SetVector("_Glow", dataGlow);
                                    fxMatGlow[matIndex].SetVector("_GlowDirection", Vector4.zero);
                                    cbHighlight.DrawRenderer(rms[k].renderer, fxMatGlow[matIndex], l);
                                }
                            }
                        }
        ...
        void ComposeSmoothBlend(Visibility smoothGlowVisibility, Visibility smoothOutlineVisibility) {
            ...
            if (renderSmoothGlow) {
                fxMatComposeGlow.SetVector("_Flip", (UnityEngine.XR.XRSettings.enabled && flipY) ? new Vector4(1, -1) : new Vector4(0, 1));
            ...
            if (renderSmoothOutline) {
                fxMatComposeOutline.SetVector("_Flip", (UnityEngine.XR.XRSettings.enabled && flipY) ? new Vector4(1, -1) : new Vector4(0, 1));
        ...
        public void SetHighlighted(bool state) {
            ...
            if (state && !ignore) {
                ...
                if (OnObjectHighlightStart != null) {
                    if (!OnObjectHighlightStart(target.gameObject)) {
                        return;
                    }
                }
            ...
            for (int k = 0; k < rmsCount; k++) {
                if (rms[k].mesh != null) {
                    ...
                    for (int m = 0; m < fxMatOutline.Length; m++) {
                        ...
                        fxMat.SetVector("_OutlineDirection", Vector4.zero);
                    ...
                    for (int m = 0; m < fxMatGlow.Length; m++) {
                        ...
                        fxMat.SetVector("_Glow2", new Vector4(outline > 0 ? outlineWidth / 100f : 0, glowAnimationSpeed, glowDithering ? 0 : 1));
